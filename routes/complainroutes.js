...
complainController.post(
  ...
  async (req, res) => {
    ...
    return res
      .status(200)
      .send({ message: "complain register successfull", complain });
  }
);
...
complainController.get("/adgpfilter", async (req, res) => {
  ...
  if (
    ...
    const complain = await complainModel.find({
      $and: [
        { createdAt: { $gt: createdAt, $lt: toDate } },
        { policestation: policestation },
        { Status: Status },
        { ComplaintCategory: ComplaintCategory },
        { District: district },
...
complainController.get("/shofilter", async (req, res) => {
  const { complainDate, Status, ComplaintCategory, Markto, io, fir } =
    req.query;
  ...
  if (Status && ComplaintCategory) {
    (filter.Status = Status), (filter.ComplaintCategory = ComplaintCategory);
  }
  if (Markto) {
    filter.Markto = Markto;
  }
  if (ComplaintCategory && Markto) {
    (filter.Markto = Markto), (filter.ComplaintCategory = ComplaintCategory);
  }
  if (Status && Markto) {
    (filter.Markto = Markto), (filter.Status = Status);
  }
